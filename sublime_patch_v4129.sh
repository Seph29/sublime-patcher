...
patch() {
	...
	echo "Patching '$1'..."
	md5sum -c <<<"BDE5ACD2C4DE4AC42E6CAE0AC5EDDE0A  $1" || exit
	printf '\x48\x31\xC0\xC3'                 | dd of=$1 bs=1 seek=$((0x003818D2)) conv=notrunc status=none
	printf '\x90\x90\x90\x90\x90'             | dd of=$1 bs=1 seek=$((0x003779C5)) conv=notrunc status=none
	printf '\x90\x90\x90\x90\x90'             | dd of=$1 bs=1 seek=$((0x003779E0)) conv=notrunc status=none
	printf '\x48\x31\xC0\x48\xFF\xC0\xC3'     | dd of=$1 bs=1 seek=$((0x00383387)) conv=notrunc status=none
	printf '\xC3'                             | dd of=$1 bs=1 seek=$((0x00381596)) conv=notrunc status=none
	printf '\xC3'                             | dd of=$1 bs=1 seek=$((0x0036B270)) conv=notrunc status=none
